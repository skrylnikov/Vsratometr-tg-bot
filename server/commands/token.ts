...
export const tokenList = async (ctx: Context) => {
  ...
  const tokenList = await Token.findAll({
    where: { id: tokenToChatList.map((x) => x.tokenId), tokenSet: null },
  });
  ...
  ctx.reply(
    `Список пользовательских токенов:\n${groupedTokenList
      .map(
        (tokenList) =>
          `${
            tokenList[0].type === "plus"
              ? "+"
              : tokenList[0].type === "minus"
              ? "-"
              : "+-"
...
export const addToken = async (ctx: Context) => {
  ...
  if (
    !chatAdmins.find((x) => x.user.id === userId && x.status === "creator") &&
    userId !== godId
  ) {
  ...
  const type =
    typeStr === "+" ? "plus" : typeStr === "-" ? "minus" : "plusAndMinus";
  ...
  const tokenInSet = await Token.findAll({
    where: { tokenSet: { [Op.not]: null } },
  });
