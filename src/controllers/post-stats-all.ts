...
export const postStatsAll = async (ctx: Context) => {
  ...
  const superTop = pipe(
    ...
    (x) => x.filter(({ plus, minus }) => plus > 2 && minus > 2 && (plus + minus) > 5),
    (x) => x.filter((_, i) => i < 10),
    (x) => x.map(({ plus, minus, url }, i) => `${convertLevel(i + 1)} score: ${plus + minus}, url: ${url}`),
    (x) => x.join('\n'),
  );
  ...
  const top = pipe(
    ...
    (x) => x.filter(({ plus }) => plus > 2),
    (x) => x.filter((_, i) => i < 10),
    (x) => x.map(({ plus, url }, i) => `${convertLevel(i + 1)} score: +${plus}, url: ${url}`),
    (x) => x.join('\n'),
  );
